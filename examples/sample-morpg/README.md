...
- Phones: under 720 px the page is one column (map, pad, command line, log, sheet), `100dvh` with safe-area padding. Coarse pointers get a touch pad while a session runs: a virtual joystick (`web/src/joystick.ts` pure, `stick.ts` DOM — the dominant axis of the thumb offset is one of wasd, sent at once and then every 140 ms while held) and buttons for f, q, Tab, i, t, p, `/`, `?`, Esc — the same `Key` values the keyboard produces. The command line is a real text field (`#cmd`) that mirrors `state.input`: it opens (and takes focus, which raises the phone keyboard) on `/`, Enter or a tap on the idle hint, owns editing and IME composition, and hands Enter/Esc to the shared handler. The ⛶ button toggles fullscreen where the API exists (not iPhone Safari).
