...
4. Dungeon: any member runs `/enter` (or interacts with the entrance NPC `D`): the party sees `entering the dungeon in 10s`, anyone may `/reject` within the window, otherwise the announcer calls `POST /dungeon/enter` and sends `dungeon.start` to the party; everyone joins the `q` channel. A solo party enters at once. `wasd` sends one `move` per adjacent cell, `f`/space attacks the weakest adjacent monster, `q` fires the skill in the facing direction, `/use <itemId>`, `/operate`. Back in town the sheet routes are `/stats <maxHp|attack|defence> [n]`, `/use <itemId>` (buffs, gear), `/equip <itemId>`, `/unequip weapon|armor`, `/talk <npcId> [questId]` and `/zone <zoneId>`; each answer replaces the side panel's sheet. The `result` frame shows the rewards; any key (or 8 s) returns to town, reloads the sheet and re-announces `pos`.
