...
- Hosted on the platform (`site` resource, `docs/decisions.md` _Static sites_): `pnpm web:build && pnpm web:config -- --config <env-file>` writes `web/dist/` with its `config.json`, then `yyt site create morpg-web --description "browser client"` (once, in the project context) and `yyt site deploy morpg-web web/dist --exclude '*.map'` publishes it (source maps stay local) at `https://dev-g.yyt.life/<slug>/` (prod: `g.yyt.life`). The build is relative (`base: "./"`), so any base path works. The page shows the token field: paste a JWT for the auth channel (dev: `POST /debug/token` on the auth stack, or the one `--batch`/`play` mints). One origin is shared by every site on that host — the client keeps the JWT in memory and never in web storage; keep it that way in a fork.
