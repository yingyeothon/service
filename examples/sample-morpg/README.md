...
| `web/`                        | The browser client (`pnpm web`, vite): `src/main.ts` wires the same `client/` core to `canvas.ts` (tiles, sprites, tweens, effects), the game HUD (`src/ui/{hud,dialog,toasts,portrait}.ts` pure view models + `src/ui/dom/*` renderers), `panels.ts` (the terminal's side/log lines, behind the debug toggle), `sheets.ts` (loads `view` + the two sheets per bundle URL), `keys.ts` (`KeyboardEvent` → readline key names), `dress.ts`/`motion.ts` (pure, tested); `vite.config.mjs` is the dev-only config/token endpoint. |
...
- The gate (`web/src/main.ts`): a black sheet covers the page until a session runs — the sign-in buttons (or, without `login`, the name/token fields and _connect_; the dev server shows both) sit in its middle with the progress line (signed in → minting/connecting → connected); it closes when the session starts and returns with the reason on quit or a failed start. A signed-in token is used once, so the way back in is a fresh sign-in.
- HUD (`web/src/ui/`): the map fills the screen and everything else floats over it, derived from `AppState` on every paint (≤ 100 ms) — no DOM-side game state. `hud.ts` → `hudModel`: the top bar (place, hp bar, level/points, target chip), the primary action (_talk_ next to an NPC, _attack_ next to a live monster — disabled with the refusal reason otherwise), _skill_/_target_ in the field, _reject_ while an entry is announced, and the icon buttons _bag_/_character_/_quests_/_party_/_chat_/_menu_; every button carries the `Key` it presses (`f`, `q`, Tab, `r`, `i`, `t`, `u`, `p`, `c`), so a tap is the keypress. `dialog.ts` → `dialogModel`: `state.overlay` as a popup — an NPC talk (portrait from `view.cast`, one tappable row per quest with its status badge and verb; a gate or the entrance is a one-row confirm), the bag (item icons from `view.icons`, count, equipped, the verb `use`/`equip`/`unequip`), the party board (members, leader, invite rows, leave), stat points, chat targets, the character sheet and the quest log (`u`); a disabled row shows its reason and does nothing. A tap goes through `pickChoice` — the same rule as the row's hotkey, so keyboard and touch never disagree. `toasts.ts`: the entry countdown with _reject_, party invites with _join_/_decline_ (the same `party` actions), the run's result (_back to town_ = `dismissResult`), a dropped run, and `error`/`event` log lines for 4 s. The menu holds fullscreen, the debug view (the terminal's side/log panels, `#side`/`#log`, hidden by default), help and quit.
- Phones: a fixed full-viewport stage with safe-area insets (the console follows `visualViewport`, so it stays above the iOS keyboard), the joystick bottom-left for coarse pointers (`web/src/joystick.ts` pure, `stick.ts` DOM — the dominant axis of the thumb offset is one of wasd, sent at once and then every 140 ms while held), the action cluster bottom-right, the icon row wraps under the status under 600 px. The command line (`#console`, a real `<input id="cmd">` mirroring `state.input`) opens on `/`, Enter or a chat row (focused inside the tap, which raises the phone keyboard), owns editing and IME composition, and hands Enter/Esc to the shared handler; a HUD button tapped while it or a popup is open closes that first (so `c` is never read as the party board's _create_ hotkey). A disabled button stays tappable and answers with its reason as a passing line (`aria-disabled`; touch has no hover). The menu's _/ command_ entry opens the line on a phone. Chat, party and whisper lines pass as toasts too, since the log is off screen. Collision: the town refuses a step onto an interactive NPC (`stepLobby` blockers) and the field a step onto a monster; players walk through each other, as the sim allows.
