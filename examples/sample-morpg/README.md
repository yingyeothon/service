...
| `cli/`                        | The terminal client (`pnpm play`): `session.ts` drives the lobby and `q` SDK clients, `state.ts`/`render.ts` are the pure model and screen, `intent.ts` the verb layer (keys → choices → the same actions), `commands.ts` the keys, overlays and slash commands, `batch.ts` the no-TTY NDJSON front-end (`--batch`), `terminal.ts` the only TTY code. |
...
Everything except `terminal.ts`/`main.ts` is pure or injected and covered by `test/cli-*.test.ts`; `cli/intent.ts` (verbs → the same actions the slash commands produce, choices with structured `ref`s, refusal codes) is the contract a GUI client reuses, and `cli/batch.ts` the machine front-end.
