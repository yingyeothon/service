...
- `todo/` is gitignored (work notes may cite infra details); history was rewritten on 2026-08-22 to drop it and every host/DB name. The identifier patterns themselves are never in the repo: regenerate `local/identifiers.txt` with `scripts/local-identifiers.sh`.
- The stateful host's public address is an identifier too, now that it is a static Elastic IP: `scripts/local-identifiers.sh` resolves the host names from `local/env/*.env` and adds the addresses they point at, so the literal IP is blocked by the same hook. Rerun it after any address change — a stale list silently stops guarding.
