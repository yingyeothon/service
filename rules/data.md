...
- Connection budget per stack (`reservedConcurrency`): auth 10, console 10 (`api`) + 1 (`expire`), match 10 (`ws`) + 8 (`authorizer`) + 4 (`worker`) + 1 (`tick`) + 1 (`debug`). Update this line when adding a function.
