...
- `env/state.<stage>.env` also carries `KV_KEK` once `scripts/get-env.sh <stage> state` has pulled it: the value is minted by `scripts/bootstrap-ssm.sh` and its durable copy is the private ops repo, never this directory alone (`rules/deployment.md` _SSM environment values_).
