...
**One provider per channel.** `userId` is `sha256(userSalt + ":" + channelId + ":" + provider + ":" + providerUserId)` — `provider` is part of the hash, so a channel that enables both GitHub and Google hands **one human two identities**, with two characters, two inventories and two party memberships. No account linking is built and none is planned. A game picks a single provider when its auth channel is created; enabling a second one later silently forks every existing player.

**`userSalt` and what a channel without one discloses.** The salt is a random per-channel value minted at creation and held only in the platform's own storage (`docs/decisions.md` _Player ids are salted per auth channel_, 2026-09-09). It exists because every other input to the hash is knowable: the channel id is the `iss` of the player's own token, the provider is one of two, and a GitHub `providerUserId` is a public sequential integer. Without a salt a `userId` is therefore **not** a one-way name — anyone holding one can confirm whose it is, and anyone holding a list of them (a leaderboard page) can walk the provider id space offline and publish the mapping.

An auth channel **whose stored secret holds no `userSalt` — every channel created before this shipped to its stage — keeps the unsalted derivation for ever**: a `userId` is the primary key of that channel's kv rows, state documents and leaderboard scores, so re-deriving it would orphan all three. Such a channel's ids stay reversible; a team that needs unlinkable ids creates a new auth channel. Nothing on the game side changes either way — `sub` is still 32 lowercase hex and no client library computes it.
