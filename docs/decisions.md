...
| DB          | **(2026-08-22 변경)** 자체 운영 MariaDB(호스트·계정 정보는 로컬 private 관리 레포 `yyt-stateful` 참조, 여기 적지 않음). stage 별 DB 1개를 콘솔이 소유(스키마/마이그레이션/쓰기). auth/topic/match 는 `SELECT` 전용 계정으로 채널 설정만 읽는다. 계정은 서비스×stage 별 1개, 자격증명은 `local/env/{service}.{stage}.env`(gitignored) → SSM. sqlite-on-S3 + 락 구성은 폐기(`packages/sqlite-s3` 제거 예정) |
| 런타임 상태 | topic 접속 목록, 매칭 풀/티켓, 세션 등 휘발성 상태는 sqlite 가 아니라 **Redis 에만** 둔다                                                                                                                                                                                                                                                                                                                 |
| Redis       | **(2026-08-22 변경)** 자체 운영 Redis 에 `ioredis`(TCP) 로 접속(`@yyt/redis`). 서비스×stage 별 ACL 유저가 `{service}:{stage}:*` 키/채널로 제한되고 prefix 도 같은 값. Lambda 컨테이너당 연결 1개 재사용, 모든 키 TTL 필수. Upstash REST 는 폐기                                                                                                                                                           |
...
| 비밀/환경   | **(2026-08-22 변경)** 자격증명은 gitignored `local/env/{service}.{stage}.env` 에만 두고 `scripts/bootstrap-ssm.sh` 로 SSM `/yyt-service/{stage}/{service}/*` 에 올린다(복구는 `scripts/get-env.sh`). 런타임 시크릿은 `serverless.yml` 에서 `${ssm:/yyt-service/{stage}/...}`                                                                                                                              |
| 테스트      | vitest. Redis/MySQL 은 인터페이스 뒤 in-memory fake(`createMemoryKv`/`createMemoryConsoleDb`), 드라이버는 mock. 실제 dev 인스턴스 통합 테스트는 `YYT_IT=1` + `local/env/*.dev.env` 가 있을 때만(opt-in). Docker 불필요. 수동 검증은 dev 스테이지 배포 + `curl`/`wscat` 스모크                                                                                                                             |
...
- **디버그 시드(2026-08-22 결정)**: dev 의 `POST /debug/channels` 는 콘솔 DB 에 써야 하므로 console 의 **dev** MySQL 계정을 SSM `/yyt-service/dev/auth/debug-mysql-user|password` 로 별도 공급한다(`bootstrap-ssm.sh dev` 가 `local/env/console.dev.env` 에서 복사). prod 에는 이 키가 없고 훅도 등록되지 않는다. 콘솔 API/CLI 가 생기면(02/05) 시드를 그쪽으로 옮기고 이 키를 제거한다.
