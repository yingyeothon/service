...
| Infra         | Region `ap-northeast-2`, `AWS_PROFILE=yyt`, stages `dev`/`prod`, domains `{console,auth,topic,match}.yyt.life` (`serverless-domain-manager`), SPA on S3 + CloudFront                                                                                                                   |
...
- Connect: `wss://topic.yyt.life/?topic={topicId}` + `Sec-WebSocket-Protocol: bearer, <auth JWT>`. Allowed if in `allowUserIds`, or (when empty) the JWT verifies against the linked auth channel.
- Messages are wrapped as `{ type:"msg", from:userId, seq, payload }` and echoed to **everyone including the sender**; `{type:"join"|"leave", userId}` on membership changes. 16 KB cap. No history.
- State: `topic:{stage}:t:{topicId}` (meta), `:conns` (set), `conn:{connId}` → topicId; all with TTL.
