...
- Schema management stays on the hand-written `CONSOLE_MIGRATIONS` (append-only DDL steps, `GET_LOCK`-serialized, run at console cold start). ORM adoption (Prisma/Drizzle/Kysely) was evaluated for migration v3 and rejected (2026-08-24; rationale corrected 2026-08-24): Prisma 7's Rust-free client removes most of the Lambda bundle/cold-start cost, so that is NOT the reason. The deciding constraints are (a) `prisma migrate deploy` is CLI-only with no programmatic API, incompatible with our in-runtime cold-start migration model — adoption requires moving migrations to deploy time first, and (b) the no-Docker test strategy relies on per-repository in-memory fakes, which PrismaClient has no first-party equivalent for. Revisit path if wanted: absorb v1–v3 via introspection, run `migrate deploy` from `scripts/deploy.sh` starting at v4, and change the test seam.
