...
| DB          | **(2026-08-22 변경)** 자체 운영 MariaDB(호스트·계정 정보는 로컬 private 관리 레포 `yyt-stateful` 참조, 여기 적지 않음). stage 별 DB 1개를 콘솔이 소유(스키마/마이그레이션/쓰기). auth/topic/match 는 `SELECT` 전용 계정으로 채널 설정만 읽는다. 계정은 서비스×stage 별 1개, 자격증명은 `services/{service}/.env.{stage}`(gitignored) → SSM. sqlite-on-S3 + 락 구성은 폐기(`packages/sqlite-s3` 제거 예정) |
