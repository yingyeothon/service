...
| DB | 서비스별 sqlite 파일 1개를 S3 에 보관. `better-sqlite3` + arm64 Lambda Layer. 쓰기 = Upstash 락(`SET NX EX 30`, 100ms 간격 최대 5s 재시도) → 다운로드 → 수정 → 업로드. 읽기 = 락 없이 ETag 조건부 다운로드(Lambda `/tmp` 캐시) |
