...
The name path (2026-09-06) adds one ordering rule for a change that spans both stacks: deploy `state` before or together with `console` — a console that already prints `api.namePath` beside a state bundle that only resolves ids advertises a path that answers 404. The reverse order is harmless (the field is simply absent).
