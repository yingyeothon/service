...
- **Usage digest**: the daily digest carries the two tables' physical size and the busiest **channels** beside the kv and leaderboard lines, with its own threshold (an eighth of kv's). Per channel rather than per player, because that is the axis with no cap — a player is bounded by 200 friends, 100 pending requests and 500 blocks, and nothing bounds how many auth channels a stage holds. Crossing the line means either far more channels than a contest has or a `social_relations_stale` sweep that has quietly stopped; an unreadable size is announced rather than read as zero.
