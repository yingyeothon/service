...
version: alpha
name: yyt-console
description: The yyt console design system, derived from an editorial workflow-software reference — white canvas, dark-ink type, hairline borders, a near-black primary button paired with a white outlined secondary, and signature colour surfaces used sparingly. Type runs Inter (standing in for Haas Grotesk) in modest weights — never bold for its own sake.
...
> Extracted from an editorial workflow-software marketing site and adopted for the yyt console (2026-09-02). Tokens are kept verbatim; the console applies them through `src/theme.ts` and the page grammar described in `README.md` — a console is not a long-scroll marketing page, so the signature surfaces appear in exactly two places (the Home hero and the shown-once secret banner) and the pricing sub-system is unused.

the reference system's marketing surfaces are quietly editorial. The base atmosphere is white canvas, dark ink type, generous whitespace, and a near-black pill CTA — nothing is fighting for attention until a section needs to. The brand voltage doesn't come from gradient washes or accent walls; it comes from **full-bleed signature cards** in `{colors.signature-coral}`, `{colors.signature-forest}`, and `{colors.surface-dark}` that punctuate long-scroll explainer pages every two or three screens. Between those signature bands, the page reads like a print magazine: a headline, supporting copy, a small image cluster, then breathing room.
...
- Secondary CTA is a `{colors.canvas}` button with `{colors.ink}` text and a hairline outline. The two together form the reference system's signature button pair.
...
- **Surface Strong** (`{colors.surface-strong}` — #e0e2e6): The light gray "Start building with the reference system" CTA banner near the footer.
...
These are the colors that carry the reference system's brand voltage. They appear as full-bleed, full-card surfaces — never as accents on a small element.
...
**`top-nav`** — A 64px-tall white bar pinned to the top of every page. the reference system wordmark sits at left; primary horizontal menu (Platform, Solutions, Resources, Enterprise, Pricing) sits center-left in `{typography.body-md}`; the right cluster carries a "Book Demo" outline link, "Sign up for free" `{component.button-primary}`, and "Log In" text link. The nav stays light on every page — the reference system does not invert the nav over dark sections.
...
**`cta-band-light`** — The light gray "Start building with the reference system" CTA strip near the footer. Background `{colors.surface-strong}` (#e0e2e6), text `{colors.ink}`, rounded `{rounded.lg}` (12px), padding `{spacing.xxl}` (48px). Carries an h2 in `{typography.display-md}` and a `{component.button-primary}`.
...
- Use `{component.button-secondary}` (white with hairline outline) as the natural pair with `{component.button-primary}`. The two together form the reference system's signature button row.
...
- Don't make `{colors.link}` (#1b61c9) the primary button color. It is the link color. The primary button is `{colors.primary}` (#181d26, near-black). Treating link-blue as the brand action is the most common mistake when reading the reference system's CSS variables.
- Don't add a gradient backdrop to the hero. the reference system's hero is white, full stop. Mesh, aurora, spotlight gradients all read as "another SaaS template" — not the reference system.
