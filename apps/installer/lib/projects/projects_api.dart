...
class ProjectsApi {
  ProjectsApi({required this.token, http.Client? client})
  ...
  Future<List<Issue>> listIssues(String projectId, {String? status}) async {
    final url =
        status == null
            ? AuthConfig.projectIssuesUrl(projectId)
            : '${AuthConfig.projectIssuesUrl(projectId)}?status=$status';
  ...
  Future<Issue> getIssue(String projectId, int number) async => Issue.fromJson(
    _object(await _get(AuthConfig.projectIssueUrl(projectId, number))),
  );
  ...
  Future<Issue> createIssue(
    ...
    _object(
      await _post(AuthConfig.projectIssuesUrl(projectId), {
        'title': title,
        'bodyMd': bodyMd,
      }),
    ),
  ...
  Future<Issue> setIssueStatus(
    ...
    _object(
      await _post(
        '${AuthConfig.projectIssueUrl(projectId, number)}/${open ? 'reopen' : 'close'}',
        null,
      ),
    ),
  );
  ...
  Future<IssueComment> addComment(
    ...
    _object(
      await _post('${AuthConfig.projectIssueUrl(projectId, number)}/comments', {
        'bodyMd': bodyMd,
      }),
    ),
  );
