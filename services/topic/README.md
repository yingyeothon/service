...
- Alarms: `ws-errors`, `http-errors`, `authorizer-lambda-errors` (Lambda Errors), `ws-throttles`, `http-throttles` (Lambda Throttles — `reservedConcurrency` is small), `authorize-errors` (log metric on `"m":"authorize error"`), `message-count` (cost guard).
