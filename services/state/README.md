...
- No Redis, no schema of its own. Console owns every migration (`state_docs`, migration `5_state_docs`; `kv_*`, migration `m0014_kvstore`) and this stack's MySQL account may only `SELECT` on `channels` and `kv_collections`, read/write `state_docs` and `kv_entries`, and `SELECT, INSERT` on `kv_keys` — no `UPDATE`/`DELETE` there, because overwriting a wrapped DEK destroys a collection for good (`rules/data.md`).
