...
refs=("$@")
[ ${#refs[@]} -gt 0 ] || refs=(--all)
# `-a` on both: one NUL byte in one blob would otherwise end the scan silently.
hits=$(git log -p --format= "${refs[@]}" -- . ':!pnpm-lock.yaml' | grep -a -E '^\+' | grep -a -cE -f local/identifiers.txt || true)
if [ "${hits:-0}" -gt 0 ]; then
  ...
  echo "  for c in \$(git rev-list ${refs[*]}); do git grep -alE -f local/identifiers.txt \$c -- . ; done" >&2
